#!/bin/bash
# You are given a file of tab seperated weather data (TSV). There is no header column in this data file.
...
# Input Format: A text file with multiple lines of tab seperated data. The first five fields have been explained above.
# Output Format: Sort the data in ascending order of the average monthly temperature in January.
# HackerRank Solution:
# sort -t$'\t' -k2 -n
# My Solution:
if [ $# -ne 1 ]; then
...
sort -t$'\t' -k2 -n "$input_file" > "$sorted_file"

# Replace the original file with the sorted output
mv "$sorted_file" "$input_file"

echo "The lines in $input_file have been sorted in ascending order of the average monthly temperature in January."